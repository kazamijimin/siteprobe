...
import { PostgresQaEvaluationRepository, QaEvaluationPersistenceCorruptionError } from "../evaluations/repository.js";
  ...
  return {
    ...
    evaluation: {
      findings: [
        ...
        { ruleId: "NO_CONSOLE_ERRORS", category: "runtime", status: "notApplicable", severity: "info", title: "No console errors", description: "n/a", evidence: { kind: "messages", recordedCount: 0, samples: [], samplesTruncated: false } },
...
describeDatabase("PostgreSQL QA evaluation persistence", () => {
  ...
  it("persists JSONB evaluation, reads through a separate repository, and enforces uniqueness", async () => {
    ...
    await expect(repository.create({ ...value, evaluation: { ...value.evaluation, summary: { critical: 1, warnings: 0, passed: 0, notApplicable: 5 } } })).resolves.toMatchObject({ created: false });
  });
