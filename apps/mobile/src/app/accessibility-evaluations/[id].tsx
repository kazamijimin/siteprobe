...
import { accessibilityStatusMessage, formatAccessibilityImpact, formatAccessibilityTimestamp, truncationMessage } from '@/features/accessibility-evaluations/presentation';
    ...
    <View style={styles.ruleCard}>
      ...
      {rule.samples.length > 0 ? (
        <View style={styles.sampleList}>
          {rule.samples.map((sample, index) => <Text selectable key={`${rule.ruleId}-${index}`} style={styles.sample}>{`Target: ${sample.target.length > 0 ? sample.target.join(' >>> ') : 'Not available'}\nFailure: ${sample.failureSummary ?? 'Not available'}`}</Text>)}
        </View>
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.content}>
          ...
          <Text style={styles.provenanceNotice}>This result was produced by automated accessibility checks on a controlled scanner fixture.{"\n"}It is separate from SiteProbe&apos;s current synthetic public scan workflow.</Text>
...
const styles = StyleSheet.create({
  ...
  ruleCard: { backgroundColor: '#FFFFFF', borderColor: '#CBD5E0', borderRadius: 12, borderWidth: 1, marginTop: 14, padding: 18 }, ruleTitle: { color: '#1A202C', fontSize: 18, fontWeight: '700', lineHeight: 25 }, ruleMeta: { color: '#2D3748', fontSize: 15, lineHeight: 23, marginTop: 8 }, sampleList: { gap: 10, marginTop: 14 }, sample: { backgroundColor: '#F7FAFC', borderRadius: 6, color: '#1A202C', fontFamily: 'monospace', fontSize: 14, lineHeight: 23, padding: 10 }, note: { color: '#4A5568', fontSize: 14, lineHeight: 21, marginTop: 12 },
